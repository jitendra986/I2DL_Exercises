...
class Relu:
    ...
    def backward(self, dout, cache):
        ...
        ########################################################################
        x = cache
        dx = np.array(dout, copy=True)  # Convert dA to a numpy array
        #dx = dx.astype(int) # Convert float array to integer array

        dx[x <= 0] = 0  # Set the elements of dx to 0 where the corresponding x values are less than or equal to 0
        ########################################################################
        #                           END OF YOUR CODE                           #
        ########################################################################
        return dx
...
def affine_forward(x, w, b):
    ...
    ########################################################################
    N = x.shape[0]
    x2 = x.reshape(N, -1)  # (100,3,2) -> (100,6)
    out = np.dot(x2, w) + b
    ########################################################################
    #                           END OF YOUR CODE                           #
    ########################################################################
